...
def evaluate(gnn, classifier, batch_start, batch_num, target_node, target_data, export_data=False):
...
#             print("Preds:", stack_output)
            
            stack_output.append(target_output)
            stack_label.append(target_labels[target_ids])
            
            if export_data:
#                 print("Feats keys:", target_feats.keys())
#                 print("Target:", target_node)
#                 print("target message:", target_message[target_node].shape)
#                 print(target_ids) 
#                 print("Used feats shape:", target_feats[target_node][target_ids].shape)
#                 print("Feats shape:", target_feats[target_node].shape)
#                 print("Out shape:", torch.softmax(target_output,dim=1).shape)
#                 feats_export.append(target_feats[target_node][target_ids].cpu())
                prob_export.append(torch.softmax(target_output, dim=1).cpu())
                
    if export_data:            
        with open('prob.pkl', 'wb') as ff:
            pickle.dump(prob_export, ff)
...
#     print("Feats shape:", len(target_data), "Pred shape:", len(stack_output))
#     print(target_data)
    
#     print(target_ids_list)
#     print(torch.softmax(stack_output, dim=1))
...
def finetuning(args, gnn, classifier, target_data):
    ...
    loss, micro_acc, macro_acc, _ = evaluate(gnn, classifier, args.train_batch2, args.test_batch2, args.target_node, target_data, export_data=False)
...
def pretrain(args, types, relations, feat_dims, label_dim, source_data, target_data):
    ...
    for epoch in range(args.epochs):
        #print(f"args.train_batch: {args.train_batch}")
        #print(f"args.val_batch: {args.val_batch}")
        #print(args.train_batch - args.val_batch)
...
#     print(">>>>>>>>>Target accuracy before fine-tuning<<<<<<<<<<")
    trg_loss1, trg_micro_acc1, trg_macro_acc1, _ = evaluate(gnn, classifier, args.train_batch2, args.test_batch2, args.target_node, target_data, export_data=True)
